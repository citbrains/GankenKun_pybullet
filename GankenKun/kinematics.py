...
class kinematics():
    ...
    def __init__(self, RobotId):
        ...
        for id in range(p.getNumJoints(self.RobotId)):
            self.index_dof[p.getJointInfo(self.RobotId, id)[12].decode(
                'UTF-8')] = p.getJointInfo(self.RobotId, id)[3] - 7
    ...
    def solve_ik(self, left_foot, right_foot, current_angles):
        ...
        joint_angles[self.index_dof['left_waist_pitch_mimic_link']
                     ] = waist_pitch
        joint_angles[self.index_dof['left_shin_pitch_link']] = knee_pitch
        joint_angles[self.index_dof['left_independent_pitch_link']
                     ] = -knee_pitch
        ...
        joint_angles[self.index_dof['left_ankle_roll_link']
                     ] = l_roll - waist_roll
        ...
        joint_angles[self.index_dof['right_ankle_roll_link']
                     ] = r_roll - waist_roll
...
if __name__ == '__main__':
    ...
    while p.isConnected():
        ...
        tar_left_foot_pos = [left_foot_pos0[0] + 0.2,
                             left_foot_pos0[1], height, 0.0, 0.0, 0.0]
        tar_right_foot_pos = [right_foot_pos0[0],
                              right_foot_pos0[1], height, 0.0, 0.0, 0.0]
        joint_angles = kine.solve_ik(
            tar_left_foot_pos, tar_right_foot_pos, joint_angles)
