...
class foot_step_planner_v1():
    ...
    def calculate(self, goal_x, goal_y, goal_th, current_x, current_y, current_th, next_support_leg, status):
        ...
        if next_support_leg == 'right':
            foot_step += [[time, current_x, -self.width +
                           current_y, current_th, next_support_leg]]
        ...
        while True:
            ...
            if next_support_leg == 'right':
                foot_step += [[time, next_x, next_y -
                               self.width, next_th, next_support_leg]]
                next_support_leg = 'left'
            elif next_support_leg == 'left':
                foot_step += [[time, next_x, next_y +
                               self.width, next_th, next_support_leg]]
...
if __name__ == '__main__':
    ...
    foot_step = planner.calculate(
        1.0, 0.0, 0.5, 0.5, 0.0, 0.1, 'right', 'start')
    for i in foot_step:
        print(i)
